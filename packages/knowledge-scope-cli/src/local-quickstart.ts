...
export type LocalDocumentImportPort = Readonly<{
  ingest: (sourcePath: string) => Promise<Readonly<{ indexRef: string; documentCount: number; chunkCount: number }>>;
}>;
export const localQuickstart = async (
  ...
  try {
    ...
    const definition = KnowledgeScopeDefinitionSchema.parse({
      packId: "local-project", version: "0.1.0", responsibility: "project-context",
    ...
    await dependencies.authoring.lock(pack); artifactsComplete = true;
    ...
    const recovery = installationId !== undefined ? ["schift-ks", "query", installationId, "--query", request.query]
