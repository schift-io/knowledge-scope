...
AI 앱 개발자나 고객별 프로젝트를 맡는 SI·에이전시를 위한 `0.2.0` 사용 가이드입니다. 먼저 공개 샘플로 **자료 연결 → 검색 → 출처 확인**을 끝내고, 같은 연결을 SDK에서 재사용합니다. Schift 계정과 모델 API 키는 필요하지 않습니다.
...
공식 사용 스킬은 [공개 저장소의 스킬 안내](https://github.com/schift-io/knowledge-scope#use-with-a-coding-agent)에서 설치 경로를 확인하세요. **이번 대화나 작업에서 사용할 프로젝트를 한 번 선택하고, 후속 질문에 같은 연결을 재사용하는 방식**입니다. 질문마다 자료를 다시 가져오는 것이 아니라 같은 연결에서 근거를 다시 검색합니다.

스킬을 전역 또는 프로젝트에 설치하는 일은 AI 도구가 지침을 발견하게 하는 일입니다. 자료가 모든 세션에 자동 활성화되는 것은 아닙니다. npm 패키지 설치만으로 스킬이 등록되는 것도 아닙니다.

```text
새 대화·분기: 선택된 프로젝트 없음
  → 이번 세션의 A 프로젝트 선택 → 새로 연결하거나 지정한 기존 연결 검사
  → 후속 질문 → A에서 다시 검색
  → B로 명시적 전환 → A의 근거 사용 중단, B 확인 후 검색
  → 세션 종료 → 세션에서 사용 중단, 저장된 연결은 유지
```

다음처럼 요청하세요.

- 처음: “이번 세션은 A 프로젝트로. `./my-documents`를 KS로 연결하고 환불 규정의 근거와 출처를 찾아줘. 외부로 업로드하지 마.”
- 이어서: “같은 자료로 배송 기간은?”
- 전환: “이제 B 프로젝트로 바꾸고, A의 자료는 답변에 사용하지 마. B의 자료는 `./project-b-documents`야.”
- 새 대화에서 재개: “새 세션에서 A 프로젝트 이어 쓰기. `./support-project`의 기존 연결을 확인하고 사용해줘.”

새 대화나 분기에서는 이전 선택을 자동으로 이어받지 않습니다. 재개를 요청한 프로젝트의 `installation.json`을 찾고 `inspect`로 현재 연결과 revision을 확인합니다. 현재 폴더나 생성 시각만 보고 가장 최근 연결을 골라 검색하지 않습니다. 선택이 불명확하거나 확인에 실패하면 자료를 추측하지 말고 확인을 요청합니다.

스킬은 현재 대화 문맥에 작업 폴더·선택한 자료·KS 상태 위치·연결 ID·revision 등 재사용에 필요한 최소 연결 정보만 유지합니다. 이를 위한 전역 세션 목록이나 원문·전체 대화 로그의 새 영구 저장소를 만들지 않습니다. 검색한 문구는 AI 도구의 대화 문맥에 들어가며, 그 도구의 기록 보존 정책은 별도로 적용됩니다. 프로젝트 전환은 이전 근거를 사용하지 않는다는 뜻이지 이미 남은 대화 내용을 삭제한다는 뜻은 아닙니다.

**이 세션 동작은 스킬의 사용 절차이지 새 런타임 격리 기능이 아닙니다.** 자동 시작·종료 후크는 없습니다. 세션을 끝내도 원문은 삭제되지 않고 연결이나 서버 권한도 해제되지 않습니다. 저장된 installation과 24시간 스냅샷 최신성 조건은 세션과 별개입니다. 명시적으로 `unmount`하면 비활성 기록이 남으며 원문은 보존됩니다. 실제 권한은 실행 계층이 검사하고, Schift Search 어댑터는 보장하지 못하는 `namespace`·`subject`·`session` 범위 지정을 거절합니다.

사용 스킬은 문서 authority·schema·평가 질문을 자동 설계하는 **Let Skill 컴파일러가 아니며**, MCP 서버도 아닙니다. 스킬 문구는 런타임 권한 검사를 대체하지 않습니다.
