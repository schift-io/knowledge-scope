...
const preview = JSON.parse(run(["prune", "--keep", "1", "--json"]));
assert.equal(preview.status, "preview");
assert.ok(preview.obsoleteInstallations.includes(first.installationId));
const pruned = JSON.parse(run(["prune", "--keep", "1", "--apply", "--plan", preview.plan, "--json"]));
assert.equal(pruned.status, "pruned");
const recovery = JSON.parse(run(["recover", "--json"]));
assert.equal(recovery.status, "recovery_preview");
const maintained = JSON.parse(run(["ask", "환불", "--json"]));
assert.ok(JSON.stringify(maintained.candidates).includes("30일"));
assert.equal(await readFile(join(source, "policy.md"), "utf8"), "");
process.stdout.write("Installed connect/ask/refresh/prune/recovery-preview: passed; application dependencies and original sources unchanged.\n");
